...
2) Final upload — загрузка финального подписанного документа
...
Не забудьте включить бин конфигурации:

@EnableConfigurationProperties(QrServiceProps.class) (например в @SpringBootApplication)
...
Примечание: FinalUploadRequest содержит SimpleMultipartFile. Если вы принимаете файл по HTTP (Spring),
чаще всего удобнее делать endpoint так:
...
Неправильные названия свойств в конфиге:
✅ qr.service.base-url, ✅ qr.service.api-key

Для final upload должен вызываться:
✅ FinalUploadClient.uploadFinal(...), а не PreCreateClient.preCreate(...)

Для обновления статуса используется:
✅ UpdateStatusClient.updateStatus(...), а не PreCreateClient.updateStatus(...)
